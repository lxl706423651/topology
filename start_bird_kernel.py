...
SYSTEM_LOAD_THRESHOLD = 110.0  # 当 1分钟 Load Average 低于此值时，认为系统收敛/空闲
...
class Logger:
    def __init__(self):
        ...
        LOG_DIR = os.path.join(EXP_DIR, "kernel") # 你可以自行分类
        os.makedirs(LOG_DIR, exist_ok=True)
        self.filename = os.path.join(LOG_DIR, f"switch_kernel_simple_{timestamp}.log")
...
def wait_for_system_idle1(phase_name):
...
def main():
    ...
    for asn in sorted_asns:
        ...
        for name, pid in as_groups[asn]:
            ...
            time.sleep(INTERVAL)
            # if processed % 1000 == 0:
            #     time.sleep(600)  # 每处理10个节点，检查一次系统负载
